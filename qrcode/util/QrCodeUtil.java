...
import com.sky.skyserver.util.qrcode.wrapper.*;
...
public class QrCodeUtil {
    ...
    public static BufferedImage drawQrInfo(QrCodeOptions qrCodeConfig, BitMatrixEx bitMatrix) {
        ...
        if (qrCodeConfig.getBgImgOptions() != null &&
                qrCodeConfig.getBgImgOptions().getBgImgStyle() == BgImgStyle.PENETRATE) {
            // 透传，用背景图颜色进行绘制时
     ...
     * @param y            目标点y坐标
     */
    private static void drawQrDotImg(QrCodeOptions qrCodeConfig, DrawStyle drawStyle, Graphics2D g2,
     ...
     * @param y            目标点y坐标
     */
    private static void drawGeometricFigure(QrCodeOptions qrCodeConfig, DrawStyle drawStyle,
        ...
        if (row2 && col2 && bitMatrix.getByteMatrix().get(x + 1, y + 1) == 1 &&
                drawStyle.expand(ExpandType.SIZE4)) {
            // 四个相等
        ...
        } else if (row2 && drawStyle.expand(ExpandType.ROW2)) {
            // 横向相同
            bitMatrix.getByteMatrix().set(x + 1, y, 0);
        ...
        } else if (col2 && drawStyle.expand(ExpandType.COL2)) {
            // 列的两个
            bitMatrix.getByteMatrix().set(x, y + 1, 0);
     ...
     * @param y            目标点y坐标
     */
    private static void drawSpecialImg(QrCodeOptions qrCodeConfig, DrawStyle drawStyle, Graphics2D g2,
                                       BitMatrixEx bitMatrix, int leftPadding, int topPadding, int infoSize, int x, int y) {
